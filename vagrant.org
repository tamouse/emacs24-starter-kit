* Vagrant User Personal Emacs Settings

  The vagrant user running in a VM will not have a display for their
  use, as it runs in a terminal. Getting rid of all the cruft of
  servers, colour themes, and so on.
...
- markdown mode
  #+begin_src emacs-lisp
    (startup-kit-install-if-needed 'markdown-mode)
